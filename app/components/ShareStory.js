...
export default function ShareStory({ session, user, prs = [], onClose }) {
  ...
  async function draw(ctx, canvas) {
    ...
    // Le SVG est blanc, on le teinte avec la couleur du thème via canvas temp
    const logoImg = new Image()
    logoImg.src = '/logo_typo.svg'
    await new Promise(resolve => {
      logoImg.onload = () => {
        // Dessiner le SVG dans un canvas temporaire pour le coloriser
        ...
        // Appliquer la couleur via multiply
      ...
      logoImg.onerror = () => {
        // Fallback texte si le SVG ne charge pas
