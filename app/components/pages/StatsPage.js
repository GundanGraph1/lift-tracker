...
import { MUSCLE_COLORS, MUSCLE_GROUPS, getMuscleColor, getMuscleLabel, BADGES, normalize, exerciseBelongsToMuscle, ALL_EXERCISES, isBW, calcBWSetVolume } from '../../../lib/constants'
    ...
    for (const s of sorted) {
      ...
      for (const ex of (s.exercises||[])) {
        ...
        vol += ex.sets.reduce((b,st) => {
          if (isBW(ex.name)) {
            if (ex.unilateral) {
              return b + calcBWSetVolume(ex.name, parseFloat(st.rL||st.r)||0, parseFloat(st.wL||st.w)||0, bw) + calcBWSetVolume(ex.name, parseFloat(st.rR||st.r)||0, parseFloat(st.wR||st.w)||0, bw)
            }
            return b + calcBWSetVolume(ex.name, parseFloat(st.r)||0, parseFloat(st.w)||0, bw)
          }
          if (ex.unilateral) return b+(parseFloat(st.rL||st.r)||0)*(parseFloat(st.wL||st.w)||0)+(parseFloat(st.rR||st.r)||0)*(parseFloat(st.wR||st.w)||0)
