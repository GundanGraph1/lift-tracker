...
// Exercices poids de corps — 2 catégories
// FULL = tu soulèves 100% de ton poids (volume = reps × (pdc + lest))
// PARTIAL = tu soulèves une fraction seulement (volume = reps × lest, 0 si pas de lest)
export const BW_FULL = [
  'Traction','Tractions','Pull-Up','Pull Up','Chin-Up','Chin Up',
  'Dips','Dip','Muscle-Up','Muscle Up','Pistol Squat',
]
export const BW_PARTIAL = [
  'Pompes','Push-Up','Push Up',
  'Relevé de jambe','Relevé de Jambes','Leg Raise',
  'Crunch','Crunches','Ab Wheel','Roue abdominale',
  'Planche','Plank','Mountain Climber','Burpee','Burpees',
  'L-Sit','Hollow Hold','Superman',
  'Gainage','Pike Push-Up','Handstand Push-Up',
]
export const BW_EXERCISES = [...BW_FULL, ...BW_PARTIAL]

function normBW(s) { return (s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'') }

export function isBW(name) {
  if (!name) return false
  const n = normBW(name)
  return BW_EXERCISES.some(bw => n.includes(normBW(bw)))
}

// Exos où tu soulèves 100% de ton poids → volume inclut pdc
export function isBWFull(name) {
  if (!name) return false
  const n = normBW(name)
  return BW_FULL.some(bw => n.includes(normBW(bw)))
}

// Calcul volume d'une série BW
// bwFull → reps × (bodyWeight + lest)
// bwPartial → reps × lest (0 si pas de lest)
export function calcBWSetVolume(exName, reps, weight, bodyWeight) {
  if (isBWFull(exName)) return reps * ((bodyWeight || 0) + (weight || 0))
  return reps * (weight || 0) // partial → lest uniquement
}
...
export const MUSCLE_SHORTCUTS = [
  ...
  { key: 'Push',     label: 'Push',     muscles: ['Pectoraux','Épaules','Triceps'] },
  { key: 'Pull',     label: 'Pull',     muscles: ['Dos','Biceps'] },
  { key: 'Legs',     label: 'Legs',     muscles: ['Jambes','Quadriceps','Ischio','Fessiers'] },
...
// Aliases musculaires — toutes les variantes en base vers la clé canonique
...
// Build reverse map: exercise name (normalized) → muscle group(s)
const _exMuscleMap = {}
for (const [muscle, exos] of Object.entries(ALL_EXERCISES)) {
  for (const ex of exos) {
    const key = normalize(ex)
    if (!_exMuscleMap[key]) _exMuscleMap[key] = []
    if (!_exMuscleMap[key].includes(muscle)) _exMuscleMap[key].push(muscle)
  }
}

// Get muscle groups for an exercise name
// Checks: built-in map → custom_exercises muscle_group → session muscle tag
export function getExerciseMuscles(exName, customExercises) {
  const key = normalize(exName)
  // 1) Built-in map
  if (_exMuscleMap[key]) return _exMuscleMap[key]
  // 2) Custom exercises with muscle_group
  if (customExercises) {
    const ce = customExercises.find(c => normalize(c.name) === key)
    if (ce && ce.muscle_group) return [ce.muscle_group]
  }
  return []
}

// Check if an exercise belongs to a canonical muscle group (handles aliases)
export function exerciseBelongsToMuscle(exName, muscle, customExercises) {
  const groups = getExerciseMuscles(exName, customExercises)
  if (!groups.length) return false
  // Direct match
  if (groups.includes(muscle)) return true
  // Check aliases: does the exercise muscle match any alias of the target?
  const aliases = MUSCLE_ALIASES[muscle] || [muscle]
  return groups.some(g => aliases.includes(g))
}
